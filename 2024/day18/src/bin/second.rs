...
fn try_path(mut grid: Array2<bool>, size: usize) -> Option<usize> {
...
fn solve(coordinates: &[(u8, u8)], size: usize, amount: usize) -> (usize, usize) {
    let mut grid = draw_grid(coordinates, size, amount);
    assert!(try_path(grid.clone(), size).is_some());
    for offset in amount.. {
        let (x, y) = coordinates[offset];
        grid[[y as usize, x as usize]] = false;
        if try_path(grid.clone(), size).is_none() {
...
mod tests {
    ...
    #[test_case(&[(5,4),(4,2),(4,5),(3,0),(2,1),(6,3),(2,4),(1,5),(0,6),(3,3),(2,6),(5,1),(1,2),
                    (5,5),(2,5),(6,5),(1,4),(0,4),(6,4),(1,1),(6,1),(1,0),(0,5),(1,6),(2,0)], 7, 12 => (6, 1))]
    fn test_solve(input: &[(u8, u8)], size: usize, amount: usize) -> (usize, usize) {
