...
fn do_the_thing(input: &str) -> IResult<&str, usize> {
    ...
    Ok((input, games.into_iter().filter_map(|game| {
        if game.glimpses.into_iter().all(|glimpse| 
            glimpse.red <= 12 && glimpse.green <= 13 && glimpse.blue <= 14) {
            Some(game.id)
            } else {
                None
            }
