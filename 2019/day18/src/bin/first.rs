...
fn string_to_data(string: &str) -> (TileMap, MapIndex, Point) {
    let mut x = 0;
    let mut y = 0;
...
fn find_keys(map: &TileMap, start: Point) -> Vec<(Point, usize)> {
    let mut map = map.clone();
    let mut options = Vec::new();
    let mut expand_from = vec![start];
    let mut steps = 0;
...
fn approx_min_steps(map: &TileMap, index: &MapIndex, start: Point, steps: usize) -> usize {
...
fn find_min_steps(map: &TileMap, index: &MapIndex, start: Point, steps: usize, mut bound: usize, depth: usize) -> usize {
